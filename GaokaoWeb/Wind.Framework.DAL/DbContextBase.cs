...
namespace Wind.Framework.DAL
{
    ...
    public class DbContextBase : DbContext, IDataRepository, IDisposable
    {
        public DbContextBase(string connectionString):base(connectionString) {
            ...
            bool flag =  this.Database.CreateIfNotExists();
        }
        ...
        internal void WriteAuditLog()
        {
            ...
            foreach (var dbEntry in this.ChangeTracker.Entries<ModelBase>().Where(p => p.State == EntityState.Added || p.State == EntityState.Deleted || p.State == EntityState.Modified))
            {
                ...
                if (auditableAttr == null)
                    continue;
                Operater model = new Operater(){ Name="未知", IP="" };
                if (WCFContext.Current!= null && WCFContext.Current.Operater!=null ) {
                    model = WCFContext.Current.Operater;
                }
                var operaterName = model.Name;
